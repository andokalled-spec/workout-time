...
(function (global) {
  const PlanRunnerPrototype = {
    ...
    _runCurrentPlanBlock: async function _runCurrentPlanBlock() {
      ...
      const viewItem = (entry && item && entry.overrides)
        ? { ...item, ...entry.overrides }
        : item;
    ...
    _planAdvance: function _planAdvance(completion = {}) {
      ...
      const viewUpcoming = (upcomingEntry && upcomingItem && upcomingEntry.overrides)
  ...
  items.forEach((item, itemIndex) => {
    ...
    for (let set = 1; set <= sets; set += 1) {
      ...
      // For the last set, append microsets according to intensity technique
      if (intensity === 'dropset') {
        ...
        timeline.push({ itemIndex, set, microIndex: 1, restSecOverride: 0,
                        overrides: { justLift: true, progressionKg: 0, perCableKg: w80 } });
        const w70 = Math.max(0, perCableKg * 0.7);
        timeline.push({ itemIndex, set, microIndex: 2, restSecOverride: restSec,
        ...
        timeline.push({ itemIndex, set, microIndex: 1, restSecOverride: 15,
